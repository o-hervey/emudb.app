...
function ProviderIcon({ provider }: { provider: 'github' | 'discord' }) {
  ...
  return (
    <svg width="16" height="16" viewBox="0 0 127.14 96.36" fill="currentColor" aria-hidden="true" className="shrink-0">
  ...
  return (
    <div className="space-y-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <AuthInput label="Username" type="text" value={username} onChange={setUsername} required minLength={3} />
        <AuthInput label="Email" type="email" value={email} onChange={setEmail} required />
        <AuthInput label="Password" type="password" value={password} onChange={setPassword} required minLength={8} />
        <AuthInput label="Confirm Password" type="password" value={confirm} onChange={setConfirm} required minLength={8} />
    ...
    <div className="min-h-[60vh] flex items-start justify-center px-4 py-16">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-[var(--color-text-muted)] mt-1">Join EmuDB to rate and submit listings.</p>
        </div>
