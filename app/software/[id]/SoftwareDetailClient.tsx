...
type TabKey = 'overview' | 'platforms' | 'systems' | 'hardware';
...
function ExternalLink({ href, label }: { href: string; label: string }) {
  return (
    <a
      ...
      className="inline-flex items-center gap-2 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-2 text-sm text-[var(--color-text)] hover:border-[var(--color-accent)] hover:text-[var(--color-accent)] transition-colors"
    >
      <span>{label}</span>
...
function SimilarItem({ software }: { software: SoftwareListing }) {
  return (
    <Link
      ...
      className="flex items-center gap-3 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] p-3 hover:border-[var(--color-accent)] transition-colors"
    >
...
export default function SoftwareDetailClient({ initialData }: { initialData: SoftwareDetail }) {
  const id = initialData.id;
  const { user } = useAuth();
  const [software, setSoftware] = useState<SoftwareDetail | null>(initialData);
  ...
  const sortedHardware = [...software.hardware].sort((a, b) => {
    ...
    return manufacturer !== 0 ? manufacturer : a.name.localeCompare(b.name);
  });
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 py-8">
      ...
      <div className="space-y-0">
          ...
          <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:gap-7">
            ...
            <div className="flex-1 min-w-0 sm:py-1">
              <div className="flex flex-wrap items-center gap-2 mb-1">
                ...
                {user && (
                  <button
                    ...
                    className="sm:ml-auto px-3 py-1 rounded-lg border border-[var(--color-border)] text-xs text-[var(--color-text-muted)] hover:border-[var(--color-border-strong)] hover:text-[var(--color-text)] transition-colors"
                  >
              ...
              {software.description && (
                <p className="text-sm text-[var(--color-text-muted)] leading-relaxed max-w-4xl">{software.description}</p>
              )}
              {hasExternalLinks && (
        ...
        {/* Tab bar */}
        <div className="flex overflow-x-auto border-b border-[var(--color-border)] mb-6 -mt-2">
          {tabs.map(({ key, label }) => (
            <button
              type="button"
              key={key}
        ...
        {activeTab === 'hardware' && (
          <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6">
            {sortedHardware.length === 0 ? (
              <p className="text-sm text-[var(--color-text-muted)]">No hardware information available.</p>
            ) : (
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {sortedHardware.map((h) => (
                  <Link
                    key={h.id}
                    href={`/browse?hardware=${h.id}`}
                    className="group rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] px-4 py-3 hover:border-[var(--color-accent)] transition-colors"
                  >
                    <p className="text-sm font-medium text-[var(--color-text)] group-hover:text-[var(--color-accent)] transition-colors line-clamp-2">
                      {h.name}
                    </p>
                    {h.manufacturer && (
                      <p className="mt-1 text-xs text-[var(--color-text-muted)]">{h.manufacturer}</p>
                    )}
                  </Link>
                ))}
              </div>
        ...
        <div className="mt-8 grid gap-4 lg:grid-cols-3">
          {user ? (
            <section className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-5">
              <h2 className="text-sm font-semibold text-[var(--color-text)] mb-3">Add to a list</h2>
          ...
          {software.similar.length > 0 && (
            <section className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-5 lg:col-span-2">
              <h2 className="text-sm font-semibold text-[var(--color-text)] mb-3">Similar</h2>
